        ...
        <div class="card-body p-4">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              />
              <div class="form-text">
                Chỉ chứa chữ cái (viết hoa) và số, không dấu, không khoảng trắng.
              </div>
            ...
            <div class="col-md-6">
              <label for="promoDiscountType" class="form-label"
              ...
              <div v-if="validationErrors.discountType" class="invalid-feedback">
                {{ validationErrors.discountType }}
              </div>
            </div>
            <div class="col-md-6">
              <label for="promoDiscountValue" class="form-label"
              ...
              <div
                v-if="validationErrors.discountValue || validationErrors.validPercentage"
            ...
            <div class="col-md-6">
              <label for="promoStartDate" class="form-label"
              ...
              <div
                v-if="validationErrors.startDate || validationErrors.endDateAfterStartDate"
                class="invalid-feedback"
              >
                {{ validationErrors.startDate || validationErrors.endDateAfterStartDate }}
              </div>
            ...
            <div class="col-md-6">
              ...
              <input
                ...
                class="form-control"
                :class="{
                  'is-invalid': validationErrors.endDate || validationErrors.endDateAfterStartDate,
                }"
                id="promoEndDate"
              ...
              <div
                v-if="validationErrors.endDate && !validationErrors.endDateAfterStartDate"
                class="invalid-feedback"
              >
            ...
            <div class="col-md-6">
              ...
              <input
                ...
                :disabled="submitting"
              />
              <div v-if="validationErrors.maxUsage" class="invalid-feedback">
            ...
            <div class="col-md-6">
              ...
              <input
                ...
                :disabled="submitting"
              />
              <div v-if="validationErrors.minOrderValue" class="invalid-feedback">
...
const fetchPromotionData = async () => {
  ...
  try {
    ...
    selectedTiers.value = promo.targetTiers
      ? promo.targetTiers
          .split(",")
          .map((t) => t.trim())
          .filter((t) => t)
    ...
    initialError.value = `Không thể tải dữ liệu khuyến mãi. Lỗi: ${
...
const validateForm = () => {
  ...
  if (!formData.code) {
    validationErrors.code = "Mã code là bắt buộc.";
    isValid = false;
  } else if (!/^[A-Z0-9]+$/.test(formData.code)) {
    validationErrors.code = "Mã chỉ được chứa chữ cái (viết hoa) và số.";
    isValid = false;
  }

  if (formData.name && formData.name.length > 255) {
    validationErrors.name = "Tên không được vượt quá 255 ký tự.";
    isValid = false;
  }

  if (!formData.discountType) {
    validationErrors.discountType = "Loại giảm giá là bắt buộc.";
    isValid = false;
  }
  if (formData.discountValue === null || formData.discountValue === undefined) {
    validationErrors.discountValue = "Giá trị giảm là bắt buộc.";
    isValid = false;
    ...
    //   validationErrors.validPercentage = "Giá trị giảm phần trăm không được lớn hơn 100.";
    //   isValid = false;
    // }
  } else if (formData.discountType === "PERCENTAGE" && formData.discountValue > 100) {
    validationErrors.discountValue = "Giá trị giảm phần trăm không được lớn hơn 100.";
    isValid = false;
  }

  if (!formData.startDate) {
    validationErrors.startDate = "Ngày bắt đầu là bắt buộc.";
    isValid = false;
  }
  if (!formData.endDate) {
    validationErrors.endDate = "Ngày kết thúc là bắt buộc.";
    isValid = false;
    ...
    try {
      ...
      if (isNaN(start.getTime())) {
        validationErrors.startDate = "Ngày bắt đầu không hợp lệ.";
        isValid = false;
      }
      if (isNaN(end.getTime())) {
        validationErrors.endDate = "Ngày kết thúc không hợp lệ.";
        isValid = false;
      }
      if (!isNaN(start.getTime()) && !isNaN(end.getTime()) && end <= start) {
        validationErrors.endDateAfterStartDate = "Ngày kết thúc phải sau ngày bắt đầu.";
        isValid = false;
      }
    } catch (e) {
      validationErrors.startDate = "Định dạng ngày giờ không hợp lệ.";
      isValid = false;
      validationErrors.endDate = "Định dạng ngày giờ không hợp lệ.";
      isValid = false;
    }
  }

  if (
    formData.maxUsage !== null &&
    formData.maxUsage !== "" &&
    (!Number.isInteger(Number(formData.maxUsage)) || Number(formData.maxUsage) < 1)
  ) {
    validationErrors.maxUsage = "Lượt sử dụng tối đa phải là số nguyên lớn hơn hoặc bằng 1.";
    isValid = false;
  }
  if (
    ...
    validationErrors.minOrderValue = "Giá trị đơn tối thiểu không được âm.";
    isValid = false;
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (err.response) {
      ...
      submitError.value =
        errorData.message ||
        `Đã có lỗi xảy ra khi ${isEditMode.value ? "cập nhật" : "thêm"} khuyến mãi.`;
      ...
      if (errorData.errors) {
        ...
        if (errorData.message?.toUpperCase().includes("CODE")) {
          validationErrors.code = errorData.message || "Mã khuyến mãi đã tồn tại.";
        }
      }
    } else {
